...
// linearizeRoute returns an [api.Route] which can linearize PDFs.
func linearizeRoute(engine gotenberg.PdfEngine) api.Route {
	return api.Route{
		Method:      http.MethodPost,
		Path:        "/forms/pdfengines/linearize",
		IsMultipart: true,
		Handler: func(c echo.Context) error {
			ctx := c.Get("context").(*api.Context)
			...
			// Alright, let's linearize the PDFs.
			outputPath := ctx.GeneratePath(".pdf")

			err = engine.Linearize(ctx, ctx.Log(), inputPaths, outputPath)
			if err != nil {
				return fmt.Errorf("linearize PDFs: %w", err)
			}

			// So far so good, the PDFs are linearized into one unique PDF.
			// Now, let's check if the client want to convert this result PDF
			// to specific PDF formats.
			zeroValued := gotenberg.PdfFormats{}
			if pdfFormats != zeroValued {
...
// thumbnailRoute returns an [api.Route] which can thumbnail PDFs.
func thumbnailRoute(engine gotenberg.PdfEngine) api.Route {
	return api.Route{
		Method:      http.MethodPost,
		Path:        "/forms/pdfengines/thumbnail",
		IsMultipart: true,
		Handler: func(c echo.Context) error {
			ctx := c.Get("context").(*api.Context)
			...
			err := ctx.FormData().
				...
				Validate()
			if err != nil {
				return fmt.Errorf("validate form data: %w", err ,page)
			}
			...
			// Alright, let's thumbnail the PDFs.
			outputPath := ctx.GeneratePath(".jpg")

			err = engine.Thumbnail(ctx, ctx.Log(), inputPaths, outputPath, page)
			if err != nil {
				return fmt.Errorf("thumbnail PDFs: %w", err)
			}
...
// pngRoute returns an [api.Route] which can png PDFs.
func pngRoute(engine gotenberg.PdfEngine) api.Route {
	return api.Route{
		Method:      http.MethodPost,
		Path:        "/forms/pdfengines/png",
		IsMultipart: true,
		Handler: func(c echo.Context) error {
			ctx := c.Get("context").(*api.Context)
			...
			// Alright, let's png the PDFs.
			outputPath := ctx.GeneratePath(".png")

			err = engine.PNG(ctx, ctx.Log(), inputPaths, outputPath, page, monochrome)
			if err != nil {
				return fmt.Errorf("PNG PDFs: %w", err)
			}
