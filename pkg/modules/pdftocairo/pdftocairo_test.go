...
func TestPdfToCairo_Descriptor(t *testing.T) {
	descriptor := new(PdfToCairo).Descriptor()

	actual := reflect.TypeOf(descriptor.New())
	expect := reflect.TypeOf(new(PdfToCairo))
...
func TestPdfToCairo_Provision(t *testing.T) {
	engine := new(PdfToCairo)
...
func TestPdfToCairo_Validate(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			binPath:     os.Getenv("PDFTOCAIRO_BIN_PATH"),
			expectError: false,
		},
	} {
...
func TestPdfToCairo_Merge(t *testing.T) {
	for _, tc := range []struct {
		scenario    string
		ctx         context.Context
		inputPaths  []string
...
func TestPdfToCairo_Convert(t *testing.T) {
	engine := new(PdfToCairo)
